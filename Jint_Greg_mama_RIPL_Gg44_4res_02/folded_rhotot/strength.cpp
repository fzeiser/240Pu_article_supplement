{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   6.637,10,2.812e-09,6.470e-06);
	ifstream strengthfile("strength.nrm");
	float strength[71],strengtherr[71],energyerr[71];
	float energy[701],trans[701];
	int i = 0;
   float a0 =  -0.7625;
   float a1 =   0.1000;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-70] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(71,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.227,1.941e-06,"^{xx}Yy");
	c1->Update();
